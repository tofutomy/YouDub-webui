...
class PipelineRunner:
    ...
    def _restore_cached_stage(self, stage: str, task: dict | None) -> None:
        ...
        if stage == "translate":
            ...
            fix = session / "metadata" / f"translation.fix.{source.target_language}.json"
            if fix.exists():
                self.artifacts.translation_file = fix
            else:
    ...
    def _asr(self, task: dict) -> None:
        ...
        if asr_model.startswith("qwen3asr:"):
            from .adapters.qwen3_asr import recognize_speech, release_model
